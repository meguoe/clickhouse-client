...
exports.defaultOperator = function(config) {
  ...
  config.registerOperator('like_in', ({ field, value }) => {
    if (Array.isArray(value) && value.length > 0) {
      const sql = value.map(v => `${field} like '%${v}%'`);
      return { sql: sql.join(' OR '), arg: [] };
    }
    return {
      sql: '1 <> 1',
      arg: [],
  ...
  config.registerOperator('ip_prefix', ({ field, value }) => {
    const values = Array.isArray(value) ? value : [ value ];
    const placeholders = values.map(() => 'ip_prefix(?)').join(', ');
  ...
  config.registerOperator('match', ({ field, value }) => {
    if (Array.isArray(value) && value.length > 0) {
      const sql = `match(${field}, '(${value.join('|')})')`;
      return { sql, arg: [] };
    }
    return {
      sql: '1 <> 1',
  ...
  config.registerOperator('startwith_in', ({ field, value }) => {
    if (Array.isArray(value) && value.length > 0) {
      const sql = value.map(v => `${field} like '${v}%'`);
      return { sql: sql.join(' OR '), arg: [] };
    }
  ...
  config.registerOperator('endwith_in', ({ field, value }) => {
    if (Array.isArray(value) && value.length > 0) {
      const sql = value.map(v => `${field} like '%${v}'`);
      return { sql: sql.join(' OR '), arg: [] };
    }
  ...
  config.registerOperator('ipin_range', ({ field, value }) => {
    if (Array.isArray(value) && value.length > 0) {
      return { sql: 'arrayExists(cidr -> isIPAddressInRange(??, cidr), [?])', arg: [ field, value ] };
    }
    return {
      sql: '1 <> 1',
      arg: [],
  ...
  config.registerOperator('array_like', ({ field, value }) => ({
    sql: 'arrayExists(x -> x LIKE ?, ??)',
    arg: [`%${value}%`, field],
  }));
