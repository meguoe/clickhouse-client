...
function parseCSVStream(s = new Set()) {
  ...
  return through(function(chunk) {
    ...
    const plen = (isFirst && strarr.length - 1 || strarr.length) - parsed.length;
...
class QueryCursor {
  ...
  _getBodyForInsert() {
    ...
    if (isFirstElObject) {
      const m = query.match(INSERT_FIELDS_MASK);
      if (m) {
  ...
  _getReqParams() {
    ...
    if (data && data.params) {
      ...
      Object.keys(data.params).forEach(k => {
        ...
        const value = encodeValue(false, data.params[k], 'TabSeparated');
